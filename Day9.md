...
FeatureNutriScope (MVP)NutriScope EnhancedFood Database20 foods68 foods (20 original + 48 new)Nutrients Tracked10 (Calories, Protein, Carbs, Fat, Fiber, Iron, Calcium, Vit C, Vit D, B12)16 (adds Potassium, Magnesium, Zinc, Sodium, Vitamin A, Folate)Food LoggingManual add, editable table, remove entrySame, plus CSV Upload (drag-drop/click, fuzzy food matching, import report)ChartsMacro doughnut, Energy barSame two, upgraded (center-text totals, gradients, styled tooltips) + Micronutrient Radar chart + 7-Nutrient Balance bar chartDashboardKPI cards, Deficiencies, Excesses, Nutrient tableSame, reorganized into a tabbed layout alongside new sectionsMeal PlanningNot available2-Day Meal Planner tab — goal-based (Balanced/High Protein/Iron Focus/High Fiber), auto-generates Breakfast/Lunch/Snack/Dinner respecting diet preferenceRisk AnalysisNot availableRisk Analysis tab — overall Balance score ring + 8 pattern-based risk cards (iron, calcium, B12, protein, fiber, vitamin D, sodium, energy) with Watch/Monitor/Stable badgesRecommendationsAdditions, Swaps, Portion tips (3 types)Same 3 + 3 new types: Timing & Pattern, Nutrient Pairing (absorption science), Priority Focus (severity-ranked)DisclaimerSmall footer note onlyProminent disclaimer banner above dashboard, educational-use framingData SourcesNot disclosedSources tab — cites IFCT 2017, USDA FoodData Central, Mifflin-St Jeor, ICMR-NIN RDAs, NIH ODS, WHO/FAO + stated limitationsNavigationSingle scrolling pageTabbed interface (Dashboard / Meal Planner / Risk Analysis / Sources)Design/TechDark theme, Chart.js, single HTML file, no backendSame design language extended, still single HTML file, no backend
